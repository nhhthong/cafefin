...
- **Auth**: none
- **Rate limit**: 5 attempts / 1 minute, keyed `(email, client_ip)` — exceeding it returns `429`
  before credentials are even checked (§ 4)
...
sequenceDiagram
    ...
    H->>L: tryConsume(email, clientIp)

    alt bucket for (email, clientIp) is exhausted
        L-->>H: not consumed
        H-->>Client: 429 "too many login attempts, try again later"
...
1. **Spend one rate-limit token before touching credentials.** `LoginRateLimiter.tryConsume(email,
   clientIp)` — a Bucket4j `Bucket`, one per `(email, client_ip)` pair, capacity 5, refilling all at
   once every 1 minute (`refillIntervally`, not a gradual trickle: exhaust it and you wait out the
   *whole* window). This runs before `findByEmail`, so an exhausted bucket short-circuits straight
   to `429` — a client hammering one account can't even reach the password check, let alone use its
   timing to guess whether the email exists. Buckets live in a plain in-memory map on the JVM
   heap — correct with exactly one `cafefin-api` instance, but each additional instance behind a
   load balancer would keep its own map and enforce the limit independently, so an attacker spread
   across `N` instances effectively gets `N × 5` attempts per window. See the Errors row below for
   what a tripped limit actually returns.
2. **Look up the user and verify the password in one step.**
...
3. **Sign the access token.** `JwtService.issueAccessToken` sets `sub` to the user's UUID and signs
...
4. **Generate the refresh token — never store it raw.** 32 bytes from `SecureRandom`, base64-encoded
...
5. **Persist the refresh token with a fresh `family_id`.** Every login starts a new rotation family
   (`UUID.randomUUID()`). A later `/refresh` call rotates within that same family; if an
   already-used (revoked) token from that family is ever replayed, the whole family gets revoked at
   once — breach detection, implemented in `POST /api/v1/auth/refresh`.
6. **Respond with both tokens.** The client stores the access token for `Authorization: Bearer
   ...
   token via `POST /api/v1/auth/refresh`.

---

## 4. Errors

Bodies below are real captured responses (`LoginEndpointTest`/`LoginRateLimitTest` runs and a live
`curl`), not invented.
...
| More than 5 login attempts for the same `(email, client_ip)` within 1 minute | `429` | `{"detail":"too many login attempts, try again later","instance":"/api/v1/auth/login","status":429,"title":"Too Many Requests"}`, plus a `Retry-After` header (seconds until the bucket refills, e.g. `Retry-After: 60`) |
